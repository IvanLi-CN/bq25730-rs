...
fn test_read_charger_status() -> Result<(), Error<ErrorKind>> {
    ...
    assert!(
        !status.fault_flags.contains(
            bq25730_async_rs::registers::ChargerStatusFaultFlags::FAULT_FORCE_CONVERTER_OFF
        )
    );
    assert!(
        !status
...
fn test_read_adc_measurements() -> Result<(), Error<ErrorKind>> {
    // For 5mOhm RsnsAc
    // Corrected Config::new call
    let config_5m_ohm_ac =
        Config::new(4, SenseResistorValue::default(), SenseResistorValue::R5mOhm);
    let expectations_5m_ohm = [I2cTransaction::write_read(
        BQ25730_I2C_ADDRESS,
    ...
    let mut charger_5m_ohm =
        bq25730_async_rs::Bq25730::new(i2c_5m_ohm, BQ25730_I2C_ADDRESS, config_5m_ohm_ac);
    ...
    // For 10mOhm RsnsAc
    // Corrected Config::new call
    let config_10m_ohm_ac = Config::new(
        4,
        SenseResistorValue::default(),
        SenseResistorValue::R10mOhm,
    );
    let expectations_10m_ohm = [I2cTransaction::write_read(
        BQ25730_I2C_ADDRESS,
        vec![Register::ADCPSYS as u8],
        vec![
            0x01, // ADCPSYS (raw=1) -> 12mV
    ...
    let mut charger_10m_ohm =
        bq25730_async_rs::Bq25730::new(i2c_10m_ohm, BQ25730_I2C_ADDRESS, config_10m_ohm_ac);
