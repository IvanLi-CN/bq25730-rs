...
use bq25730_async_rs::{BQ25730_I2C_ADDRESS, data_types::{Config, SenseResistorValue}}; // Updated imports
...
fn test_set_iin_host() -> Result<(), Error<ErrorKind>> {
    ...
    let current_100ma = IinHost { milliamps: 100, rsns_ac };
    let raw_100ma = current_100ma.to_raw(); // Should be 0
    let expectations_100ma = [
        I2cTransaction::write_read(BQ25730_I2C_ADDRESS, vec![Register::IinHostMsb as u8], vec![0x80]), // Mock initial read with D7 set
        I2cTransaction::write(BQ25730_I2C_ADDRESS, vec![Register::IinHostMsb as u8, 0x80 | raw_100ma]),
    ];
    ...
    let current_200ma = IinHost { milliamps: 200, rsns_ac };
    let raw_200ma = current_200ma.to_raw(); // Should be 1
    let expectations_200ma = [
        I2cTransaction::write_read(BQ25730_I2C_ADDRESS, vec![Register::IinHostMsb as u8], vec![0x00]),
        I2cTransaction::write(BQ25730_I2C_ADDRESS, vec![Register::IinHostMsb as u8, raw_200ma]),
    ];
    ...
    let current_10000ma = IinHost { milliamps: 10000, rsns_ac };
    let raw_10000ma = current_10000ma.to_raw(); // Should be 99 (0x63)
    let expectations_10000ma = [
        I2cTransaction::write_read(BQ25730_I2C_ADDRESS, vec![Register::IinHostMsb as u8], vec![0x00]),
        I2cTransaction::write(BQ25730_I2C_ADDRESS, vec![Register::IinHostMsb as u8, raw_10000ma]),
    ];
    let i2c_10000ma = I2cMock::new(&expectations_10000ma);
    let mut charger_10000ma = bq25730_async_rs::Bq25730::new(i2c_10000ma, BQ25730_I2C_ADDRESS, config);
...
fn test_read_iin_host() -> Result<(), Error<ErrorKind>> {
    ...
    assert_eq!(
        ...
        IinHost { milliamps: 10000, rsns_ac }
    );
    charger_10000ma.i2c.done();
...
fn test_set_iin_dpm() -> Result<(), Error<ErrorKind>> {
    ...
    let current_100ma = IinDpm { milliamps: 100, rsns_ac };
    let raw_100ma = current_100ma.to_raw();
    let expectations_100ma = [
        I2cTransaction::write_read(BQ25730_I2C_ADDRESS, vec![Register::IinDpmMsb as u8], vec![0x80]),
        I2cTransaction::write(BQ25730_I2C_ADDRESS, vec![Register::IinDpmMsb as u8, 0x80 | raw_100ma]),
    ];
    let i2c_100ma = I2cMock::new(&expectations_100ma);
    let mut charger_100ma = bq25730_async_rs::Bq25730::new(i2c_100ma, BQ25730_I2C_ADDRESS, config);
    ...
    let current_200ma = IinDpm { milliamps: 200, rsns_ac };
    let raw_200ma = current_200ma.to_raw();
    let expectations_200ma = [
        I2cTransaction::write_read(BQ25730_I2C_ADDRESS, vec![Register::IinDpmMsb as u8], vec![0x00]),
        I2cTransaction::write(BQ25730_I2C_ADDRESS, vec![Register::IinDpmMsb as u8, raw_200ma]),
    ];
    let i2c_200ma = I2cMock::new(&expectations_200ma);
    let mut charger_200ma = bq25730_async_rs::Bq25730::new(i2c_200ma, BQ25730_I2C_ADDRESS, config);
    ...
    let current_10000ma = IinDpm { milliamps: 10000, rsns_ac };
    let raw_10000ma = current_10000ma.to_raw();
    let expectations_10000ma = [
        I2cTransaction::write_read(BQ25730_I2C_ADDRESS, vec![Register::IinDpmMsb as u8], vec![0x00]),
        I2cTransaction::write(BQ25730_I2C_ADDRESS, vec![Register::IinDpmMsb as u8, raw_10000ma]),
    ];
    let i2c_10000ma = I2cMock::new(&expectations_10000ma);
    let mut charger_10000ma = bq25730_async_rs::Bq25730::new(i2c_10000ma, BQ25730_I2C_ADDRESS, config);
...
fn test_read_iin_dpm() -> Result<(), Error<ErrorKind>> {
    ...
    assert_eq!(
        ...
        IinDpm { milliamps: 10000, rsns_ac }
    );
    charger_10000ma.i2c.done();
